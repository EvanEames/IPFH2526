...
def run_exercise():
    ...
    for idx, func in enumerate(functions):
        ...
        if score < 100:
            ...
            if retry == "yes":
                single_fun_script(func["function"], idx+1, func["options"], func["solutions"])
                continue
...
def single_fun_script(fun, numb, option_1, answer_1):
    print(f"Function {numb}:")
    print(fun)
    for i, opt in enumerate(option_1, start=1):
        print(f"{i}. {opt}")
    user_answers = []
    for _ in answer_1:
        user_input = input("Fill in the blank: ")
    ...
    correct = 0
    for user, solution in zip(user_answers, answer_1):
        if user == solution:
            correct += 1

    score = (correct / len(answer_1)) * 100
    print(f"Your score: {score:.2f}%")

    if score < 100:
        retry = input("Do you want to try this function again? (yes/no): ").lower()
        if retry == "yes":
            single_fun_script(fun, numb, option_1, answer_1)
